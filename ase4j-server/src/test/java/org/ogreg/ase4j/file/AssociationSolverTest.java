...
public class AssociationSolverTest {
	...
	public void testIntersection() {
		...
		equals(solver.intersection(row("2,3"), row("2")), row("2"));

        // At the beginning, overlap
		equals(solver.intersection(row("2,3"), row("1,2")), row("2"));

        // In the middle
		equals(solver.intersection(row("1,2,3"), row("2,3,4")), row("2,3"));

        // At the end, overlap
		equals(solver.intersection(row("2,3"), row("3,4")), row("3"));

        // At the end, corner case
		equals(solver.intersection(row("2,3"), row("3")), row("3"));
    }
	...
	public void testUnion() {
        ...
        // Full overlap
		equals(solver.union(row("2,3,4"), row("2,3,4")), row("2,3,4"));

        // No overlap
		equals(solver.union(row("2,3"), row("4,5")), row("2,3,4,5"));

        // Overlap in the middle
		equals(solver.union(row("2,3"), row("3,4")), row("2,3,4"));

        // At the beginning, corner case
		equals(solver.union(row("2,4"), row("2")), row("2,4"));

        // At the end, corner case
		equals(solver.union(row("4"), row("2,4")), row("2,4"));
	}
